    ...
    async def detect_language(self, text: str) -> Optional[str]:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a language detection system. Respond with ONLY the ISO 639-1 language code (e.g., 'en', 'es', 'ja'). If unsure, respond with 'unknown'."},
                    {"role": "user", "content": f"What language is this text written in? Text: {text[:500]}"}
                ],
                temperature=0,
                max_tokens=10
            )
    ...
    async def translate(self, text: str, source: str, target: str) -> str:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": f"You are a professional translator. Translate text from {source_name} to {target_name}. Maintain the original formatting and style. Only provide the translation, no explanations."},
