        ...
        if text != previous_text and text:
            ...
            try:
                (
                    ...
                    original_text,
                    cached,
                ) = await translation_engine.translate_text(text, source, target)
            except Exception as e:
            ...
            cache_status = (
                "[yellow][cached][/yellow]" if cached else "[cyan][new][/cyan]"
            )
            console.print(f"\n{cache_status}")
...
def translate(
    ...
    show_original: bool = typer.Option(
        False, "--show-original", "-o", help="Show original text before translation"
    ),
    romaji: bool = typer.Option(
        False, "--romaji", "-r", help="Show romaji reading for Japanese text"
    ),
    hiragana: bool = typer.Option(
        False, "--hiragana", "-h", help="Show hiragana reading for Japanese text"
    ),
    once: bool = typer.Option(
        False, "--once", help="Translate only once instead of continuously"
    ),
    engine: str = typer.Option(
        None,
        "--engine",
        "-e",
        help="Translation engine (google, openai, deepl, claude)",
    ),
) -> None:
    ...
    try:
        ...
        if romaji and hiragana:
            console.print(
                "[bold red]Error: Cannot use both --romaji and --hiragana options together[/bold red]"
            )
            raise typer.Exit(1)

        if once:
            console.print(
                f"[bold green]Waiting for {source} text in clipboard[/bold green] ({source} → {target}) using {engine}"
            )
        else:
            console.print(
        ...
        asyncio.run(
            translate_loop(
                source, target, show_original, romaji, hiragana, once, engine
            )
        )
...
def languages(
    engine: str = typer.Option(
        None, "--engine", "-e", help="Show languages for specific engine"
    ),
) -> None:
...
def configure(
    ...
    set_default: bool = typer.Option(
        False, "--set-default", "-d", help="Set as default engine"
    ),
) -> None:
