        ...
        try:
            ...
            if not backend.validate_config():
                ...
                self.test_completed.emit(
                    self.engine, False, "Invalid configuration - check API key"
                )
                return
            ...
            try:
                ...
                if result and result.strip():
                    ...
                    self.test_completed.emit(
                        self.engine, False, "Translation test failed - empty response"
                    )

            except Exception as e:
                logger.error(f"Translation test failed for {self.engine}: {e}")
                self.test_completed.emit(
                    self.engine, False, f"Translation test failed: {str(e)}"
                )
            finally:
                if loop:
                    try:
            ...
            self.test_completed.emit(self.engine, False, f"Connection failed: {str(e)}")
...
class EngineConfigWidget(QWidget):
    ...
    def on_test_completed(self, engine, success, message):
        ...
        logger.info(
            f"Test completed for {engine}: success={success}, message={message}"
        )
